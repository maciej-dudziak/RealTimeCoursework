...
static uint16_t mDeviceShortAddress[no_of_slaves] = { 0xFFFF, 0xFFFF, 0xFFFF };
static uint64_t mDeviceLongAddress[no_of_slaves] = { 0xFFFFFFFFFFFFFFFF, 0xFFFFFFFFFFFFFFFF, 0xFFFFFFFFFFFFFFFF };
...
uint8_t seq_no = 0x00;
/* Small buffer for send packets to check whether they were ACK'ed */
uint8_t packets_sent[5] = {0};
/************************************************************************************
*************************************************************************************
...
static uint8_t App_SendAssociateResponse(nwkMessage_t *pMsgIn, uint8_t appInstance)
{
  ...
  if(pMsg != NULL)
  {
    ...
    if(pMsgIn->msgData.associateInd.capabilityInfo & gCapInfoAllocAddr_c)
    {
      ...
      pAssocRes->assocShortAddress = 0x0001 + deviceId;
    }
    else
    {
...
static void formNwkDataPacket(nwkPacket_t* pNwkPck)
{
	...
	/* Check first  byte of received command to establish packet destination */
	Serial_GetByteFromRxBuffer(interfaceId, &readByte, &count);
	/* If we read one byte, establish the destination address and save it to the packet*/
	if ( readByte == '1' )
	{
		pNwkPck->destination_address = mDeviceShortAddress[0];
		pNwkPck->other_slave_address1 = mDeviceShortAddress[1];
		pNwkPck->other_slave_address2 = mDeviceShortAddress[2];
	}
	else if (readByte == '2' )
	{
		pNwkPck->destination_address = mDeviceShortAddress[1];
		pNwkPck->other_slave_address1 = mDeviceShortAddress[0];
		pNwkPck->other_slave_address2 = mDeviceShortAddress[2];
	}
	else if (readByte == '3' )
	{
		pNwkPck->destination_address = mDeviceShortAddress[2];
		pNwkPck->other_slave_address1 = mDeviceShortAddress[0];
		pNwkPck->other_slave_address2 = mDeviceShortAddress[1];
	}
	else
	{
	    return;
    ...
    /* Save the data into the packet */
    Serial_GetByteFromRxBuffer(interfaceId, &readByte, &count);
    pNwkPck->data = readByte;
    /* Update seq number */
    seq_no += 1;
...
static void App_BroadcastData(void)
{
	...
	nwkPacket_t data_to_send;
	/* fill its fields with the serial data */
...
static void App_TransmitUartData(uint16_t* dstAddress, nwkPacket_t* pNwkPck)
{
    ...
    if(mpPacket != NULL)
    {
        ...
        //mpPacket->msgData.dataReq.pMsdu = (uint8_t*) pNwkPck;
        ...
        FLib_MemCpy(&mpPacket->msgData.dataReq.dstAddr, (void*)dstAddress, 2);
        ...
        mpPacket->msgData.dataReq.msduLength = sizeof(nwkPacket_t);
