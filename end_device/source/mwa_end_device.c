...
static uint64_t maMyAddressLong = 0xFFFFFFFFFFFFFFFF;
static uint16_t maMyAddressShort = 0xFFFF;
...
static uint8_t App_HandleAssociateConfirm(nwkMessage_t *pMsg)
{
  ...
  if ( pMsg->msgData.associateCnf.status == gSuccess_c) 
  {
	  ...
	  if( pMsg->msgData.associateCnf.assocShortAddress >= 0xFFFE)
	  {
	    ...
	    maMyAddressLong = mExtendedAddress;
...
static void App_HandleMcpsInput(mcpsToNwkMessage_t *pMsgIn)
{
  ...
  case gMcpsDataInd_c:
	...
	/* if the node already received that packet - just ignore it */
	if( data_in.packet_seq_number != current_seq_no )
	{
	  ...
	  /* Do the action with the LED */
	//command = pMsgIn->msgData.dataInd.pMsdu[0];
